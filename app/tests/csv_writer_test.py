...
class TestCsvWriter:
    ...
    def test_write_unique_new_file(self, csv_writer, tmp_path, sample_v2_card_data):
        """Test writing unique data to a new file"""
        test_file = tmp_path / "unique_test.csv"

        csv_writer.write_unique(sample_v2_card_data[:2], test_file)
        ...
        assert rows[1]['period_start_date'] == '2025-01-04'

    def test_write_unique_existing_file_no_duplicates(self, csv_writer, tmp_path, sample_v2_card_data):
        """Test writing unique data when no duplicates exist"""
        test_file = tmp_path / "unique_existing.csv"

        # Create initial data
        csv_writer.write([sample_v2_card_data[0]], test_file)

        # Add new unique data
        new_data = [sample_v2_card_data[1]]
    ...
    def test_write_unique_existing_file_with_duplicates(self, csv_writer, tmp_path, sample_v2_card_data):
        """Test writing unique data when duplicates exist"""
        test_file = tmp_path / "unique_duplicates.csv"

        # Create initial data
        csv_writer.write(sample_v2_card_data[:2], test_file)

        # Try to add data with duplicates and one new entry
        new_data = [
            sample_v2_card_data[0],  # Duplicate
            sample_v2_card_data[2]   # New
        ]
        csv_writer.write_unique(new_data, test_file)
        ...
        start_dates = [row['period_start_date'] for row in rows]
        assert '2025-01-01' in start_dates
        assert '2025-01-04' in start_dates
        assert '2025-01-07' in start_dates
    ...
    def test_write_unique_empty_data(self, csv_writer, tmp_path, sample_v2_single_record):
        """Test writing empty data to existing file"""
        test_file = tmp_path / "unique_empty.csv"

        # Create initial data
        csv_writer.write([sample_v2_single_record], test_file)
